...
resource "aws_iam_policy" "ec2_deployer_limited_policy" {
  name = "ec2DeployerLimited-policy"
  policy = jsonencode({
    "Statement" : [
      {
        "Action" : [
...
resource "aws_iam_role_policy_attachment" "ec2-deployer-role-attachment" {
  ...
  policy_arn = aws_iam_policy.ec2_deployer_limited_policy.arn
}
...
resource "aws_alb" "application_load_balancer" {
  ...
  subnets            = [aws_subnet.lab-subnet-private-1.id, aws_subnet.lab-subnet-private-1b.id]
  security_groups    = [aws_security_group.load_balancer_security_group.id]
