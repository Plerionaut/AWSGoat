...
resource "aws_kms_key" "goat_key" {
  description             = "AWS Goat encryption key"
  deletion_window_in_days = 7
}

resource "aws_kms_alias" "goat_key_alias" {
  name          = "alias/aws-goat-key"
  target_key_id = aws_kms_key.goat_key.key_id
}
...
resource "aws_cloudwatch_log_group" "vpc_log_group" {
  ...
  retention_in_days = 7
  kms_key_id        = aws_kms_key.goat_key.arn
}
...
resource "aws_s3_bucket_server_side_encryption_configuration" "alb_logs" {
  ...
  rule {
    apply_server_side_encryption_by_default {
      kms_master_key_id = aws_kms_key.goat_key.arn
...
resource "aws_s3_bucket_versioning" "bucket_tf_files" {
  bucket = aws_s3_bucket.bucket_tf_files.id
  versioning_configuration {
    status = "Enabled"
  }
...
output "ad_Target_URL" {
  value = "${aws_lb.application_load_balancer.dns_name}:443/login.php"
}
FROM php:8.0-apache
...
RUN apt-get install -y python3 libcap2-bin fdisk net-tools netcat
...
RUN chown -R appuser:appuser /var/www/html/documents
